 ...
 *
 * All redirects use absolute application routes (/dashboard, /login) rather than
 * file paths (dashboard.html, login.html) so they resolve correctly from any
 * sub-path such as /auth/callback.
 */
...
(function() {
    ...
    // Paths that count as "an auth page" for the already-logged-in redirect.
    const AUTH_PATHS = ['/login', '/login.html', '/register', '/register.html'];
