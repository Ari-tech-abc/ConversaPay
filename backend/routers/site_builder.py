...
def _render(data: Dict[str, Any], business_id: str) -> str:
    title = escape(str((data.get("seo") or {}).get("title") or "Your site")); headline = escape(str((data.get("hero") or {}).get("headline") or title)); return f"<!doctype html><html><head><title>{title}</title></head><body><main><h1>{headline}</h1><p data-business-id='{escape(business_id, quote=True)}'></p></main></body></html>"
