...
def _consume_rate_limit(business_id: str, ip_address: str) -> None:
    ...
    allowed = result.data if isinstance(result.data, bool) else bool(result.data and result.data[0] if isinstance(result.data, list) else result.data)
    if not allowed:
...
async def create_site_lead(payload: LeadSubmission, request: Request):
    ...
    if not _origin_allowed(origin, business.data):
