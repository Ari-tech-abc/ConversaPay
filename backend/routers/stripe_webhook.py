...
def _lookup_user(customer_id):
    ...
    if result.data:return result.data[0]["user_id"]
    try:
        stripe_service._ensure_configured(); customer=stripe.Customer.retrieve(customer_id); email=customer.get("email")
        if email:
            result=supabase.table("profiles").select("user_id").ilike("email",email).limit(1).execute()
            if result.data:return result.data[0]["user_id"]
    except Exception as exc: logger.warning("Stripe customer lookup failed: %s",exc)
    return None
def _subscription_context(subscription_id):
    if not subscription_id:return {}
    try: subscription=stripe_service.retrieve_subscription(subscription_id)
    except (StripeServiceError,ValueError) as exc: logger.warning("Unable to retrieve subscription %s: %s",subscription_id,exc); return {}
    return {"id":subscription.get("id") or subscription_id,"metadata":dict(subscription.get("metadata") or {}),"status":subscription.get("status"),"current_period_end":period_end_iso(subscription.get("current_period_end")),"customer":subscription.get("customer"),"cancel_at_period_end":bool(subscription.get("cancel_at_period_end",False))}
...
def _apply_from_subscription(*,user_id,plan_type,subscription_id,customer_id,context):
    apply_subscription_state(user_id=user_id or _lookup_user(customer_id),plan_type=plan_type,subscription_status=context.get("status"),subscription_expires_at=context.get("current_period_end"),customer_id=customer_id,cancel_at_period_end=context.get("cancel_at_period_end",False),stripe_subscription_id=context.get("id") or subscription_id)
@router.post("")
    ...
    try:
        ...
        if event_type in {"checkout.session.completed","checkout.session.async_payment_succeeded"}:
            ...
            if obj.get("mode")=="subscription":
                subscription_id=obj.get("subscription") if isinstance(obj.get("subscription"),str) else None; context=_subscription_context(subscription_id); context_metadata=context.get("metadata") or {}; _apply_from_subscription(user_id=context_metadata.get("user_id") or metadata.get("user_id"),plan_type=context_metadata.get("plan_type") or metadata.get("plan_type"),subscription_id=subscription_id,customer_id=obj.get("customer"),context=context)
            ...
            subscription_id=obj.get("id"); context={"id":subscription_id,"metadata":dict(obj.get("metadata") or {}),"status":"canceled" if event_type=="customer.subscription.deleted" else obj.get("status"),"current_period_end":period_end_iso(obj.get("current_period_end")),"customer":obj.get("customer"),"cancel_at_period_end":bool(obj.get("cancel_at_period_end",False))}; _apply_from_subscription(user_id=context["metadata"].get("user_id"),plan_type=context["metadata"].get("plan_type"),subscription_id=subscription_id,customer_id=obj.get("customer"),context=context)
        elif event_type in {"invoice.paid","invoice.payment_succeeded","invoice.payment_failed"}:
            context=_subscription_context(obj.get("subscription"))
            if event_type!="invoice.payment_failed" or context.get("status") in {"canceled","unpaid","incomplete_expired"}:
                metadata=context.get("metadata") or {}; _apply_from_subscription(user_id=metadata.get("user_id"),plan_type=metadata.get("plan_type"),subscription_id=context.get("id"),customer_id=context.get("customer") or obj.get("customer"),context=context)
        elif event_type in {"charge.refunded","charge.refund.updated"}:
            customer_id=obj.get("customer"); user_id=_lookup_user(customer_id)
            if user_id: apply_subscription_state(user_id=user_id,plan_type="free",subscription_status="canceled",subscription_expires_at=None,customer_id=customer_id,cancel_at_period_end=False)
