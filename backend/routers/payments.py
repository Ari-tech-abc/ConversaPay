...
async def create_subscription_checkout_session(request: SubscriptionCreate, current_user: AuthUser = Depends(require_auth)) -> SubscriptionResponse:
    ...
    try:
        ...
        session = stripe_service.create_checkout_session_with_price(price_id=price_id, mode="subscription", customer_email=current_user.email, metadata=metadata) if price_id else stripe_service.create_checkout_session(amount=PLAN_PRICES[plan], product_name=f"ConversaPay {plan.upper()} subscription", currency="ILS", mode="subscription", customer_email=current_user.email, metadata=metadata)
    except ValueError as exc:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, str(exc)) from exc
    except StripeServiceError as exc:
        code = status.HTTP_500_INTERNAL_SERVER_ERROR if "not configured" in str(exc).lower() else status.HTTP_502_BAD_GATEWAY
...
async def create_order_checkout_session(request: dict[str, Any], current_user: AuthUser = Depends(require_auth)) -> dict[str, Any]:
    ...
    try:
        ...
        code = status.HTTP_400_BAD_REQUEST if isinstance(exc, ValueError) else status.HTTP_502_BAD_GATEWAY
        raise HTTPException(code, str(exc)) from exc
