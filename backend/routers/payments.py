...
async def create_subscription_checkout_session(request: SubscriptionCreate, current_user: AuthUser = Depends(require_auth)) -> dict[str, Any]:
    if request.user_id != current_user.user_id: raise HTTPException(status.HTTP_403_FORBIDDEN, "User ID mismatch")
    plan = (request.plan_type or "").lower()
    if plan not in PLAN_PRICES: raise HTTPException(status.HTTP_400_BAD_REQUEST, "Only PRO and PREMIUM can be purchased")
    try:
        return {**stripe_service.create_checkout_session(amount=PLAN_PRICES[plan], product_name=f"ConversaPay {plan.upper()} subscription", currency="ILS", mode="subscription", customer_email=request.email, metadata={"user_id": current_user.user_id, "plan_type": plan}), "provider": "stripe"}
    except (StripeServiceError, ValueError) as exc: raise HTTPException(status.HTTP_502_BAD_GATEWAY, "Unable to create Stripe checkout") from exc
...
async def get_profile(current_user: AuthUser = Depends(require_auth)) -> ProfileResponse:
    ...
    if not result.data: result = supabase.table("profiles").insert({"user_id": current_user.user_id, "email": current_user.email, "plan_type": "free"}).execute()
...
async def create_order_checkout_session(request: dict[str, Any], current_user: AuthUser = Depends(require_auth)) -> dict[str, Any]:
    ...
    if not business_id or not order_id: raise HTTPException(status.HTTP_400_BAD_REQUEST, "Missing required fields: business_id, order_id")
    ...
    if not order.data: raise HTTPException(status.HTTP_404_NOT_FOUND, "Order not found")
    data = order.data[0]
    try:
        session = stripe_service.create_checkout_session(amount=data["total"], product_name=f"ConversaPay order {data.get('order_number', order_id)}", currency=data.get("currency", "ILS"), mode="payment", customer_email=request.get("customer_email"), metadata={"order_id": order_id, "business_id": business.data[0]["id"]})
    except (StripeServiceError, ValueError) as exc: raise HTTPException(status.HTTP_502_BAD_GATEWAY, "Unable to create Stripe checkout") from exc
    payment = supabase.table("payments").insert({"business_id": business.data[0]["id"], "order_id": order_id, "amount": data["total"], "currency": data.get("currency", "ILS"), "status": "pending", "customer_email": request.get("customer_email"), "customer_name": request.get("customer_name"), "metadata": {"provider": "stripe", "session_id": session["id"]}}).execute()
    return {**session, "provider": "stripe", "payment_id": payment.data[0]["id"] if payment.data else None}

@router.get("/success")
async def payment_success(session_id: str | None = None) -> dict[str, Any]: return {"status": "pending_confirmation", "provider": "stripe", "session_id": session_id}

@router.get("/canceled")
async def payment_canceled() -> dict[str, str]: return {"status": "canceled", "provider": "stripe"}

@router.get("", response_model=list[PaymentResponse])
async def get_payments(business_id: str, current_user: AuthUser = Depends(require_auth)) -> list[PaymentResponse]:
    business = supabase.table("businesses").select("id").eq("business_id", business_id).eq("owner_id", current_user.user_id).execute()
    if not business.data: raise HTTPException(status.HTTP_404_NOT_FOUND, "Business not found")
    result = supabase.table("payments").select("*").eq("business_id", business.data[0]["id"]).order("created_at", desc=True).execute(); return [PaymentResponse(**item) for item in (result.data or [])]
...
async def get_payment(payment_id: str, current_user: AuthUser = Depends(require_auth)) -> PaymentResponse:
    ...
    item = result.data[0]; owner = supabase.table("businesses").select("id").eq("id", item["business_id"]).eq("owner_id", current_user.user_id).execute()
    if not owner.data: raise HTTPException(status.HTTP_403_FORBIDDEN, "Access denied")
...
async def get_wordpress_plugin(business_id: str, current_user: AuthUser = Depends(require_auth)) -> Response:
    ...
    if not plugin_path.exists(): raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, "Plugin file not found")
