...
def normalize_plan(value): value=str(value or 'free').strip().lower(); return {'professional':'pro','pro_monthly':'pro','premium_monthly':'premium','paid':'pro'}.get(value,value if value in PLANS else 'free')
def subscription_active(row):
    ...
    try: exp=datetime.fromisoformat(str(raw).replace('Z','+00:00')); return (exp if exp.tzinfo else exp.replace(tzinfo=timezone.utc))>datetime.now(timezone.utc)
...
def get_user_plan(user_id):
    ...
    requested=normalize_plan(row.get('plan_type')); return {**row,'plan_type':requested if subscription_active({**row,'plan_type':requested}) else 'free','subscription_active':subscription_active({**row,'plan_type':requested})}
