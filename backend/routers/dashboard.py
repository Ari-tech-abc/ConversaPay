...
def get_user_plan(user_id: str) -> Dict[str, Any]:
    try:
        ...
        row = result.data or {}; plan = row.get("plan_type", "free")
        ...
        logger.exception("Failed to load user plan"); return {"plan_type": "free", "email_verified": False}

def require_verified(user: AuthUser):
    if not get_user_plan(user.user_id).get("email_verified"): raise HTTPException(status_code=403, detail="email_not_verified")

def entitlements(user: AuthUser):
    plan = get_user_plan(user.user_id); return plan, dict(PLANS.get(plan["plan_type"], PLANS["free"]))

@router.get("/profile")
async def profile(current_user: AuthUser = Depends(require_auth)):
    require_verified(current_user); plan, features = entitlements(current_user); return {"user_id": current_user.user_id, "email": current_user.email, **plan, "features": features}

@router.get("/features")
async def features(current_user: AuthUser = Depends(require_auth)):
    require_verified(current_user); plan, data = entitlements(current_user); data["plan_type"] = plan["plan_type"]; data["site_builder_url"] = "https://builder.conversapay.org/" if data["site_builder"] else "/upgrade.html?feature=site_builder"; return data

@router.get("/upgrade-options")
async def upgrade_options(current_user: AuthUser = Depends(require_auth)):
    require_verified(current_user); plan, _ = entitlements(current_user); offers=[]
...
async def settings_view(current_user: AuthUser = Depends(require_auth)):
    require_verified(current_user); plan, features = entitlements(current_user); return {"plan_type":plan["plan_type"],"payme_id":plan.get("payme_id"),"whatsapp_phone":plan.get("whatsapp_phone_number_id"),"domain_limit":features["domain_limit"],"features":features}
...
async def products(current_user: AuthUser = Depends(require_auth)):
    ...
    items=[{"id":r["id"],"name":r.get("name"),"price":r.get("price",0),"stock":r.get("inventory_count",-1),"category":(r.get("metadata") or {}).get("category","")} for r in rows]; return {"products":items,"total":len(items)}

@router.get("/orders")
async def orders(current_user: AuthUser = Depends(require_auth)):
    require_verified(current_user); plan, features=entitlements(current_user); businesses=supabase.table("businesses").select("id").eq("owner_id",current_user.user_id).execute()
...
async def analytics(current_user: AuthUser = Depends(require_auth)):
    ...
    bid=businesses.data[0]["id"]; paid=supabase.table("orders").select("total").eq("business_id",bid).in_("status",["paid","shipped","delivered"]).execute().data or []; conversations=supabase.table("conversations").select("session_id").eq("business_id",bid).execute().data or []; revenue=round(sum(float(x.get("total",0)) for x in paid),2); deals=len(paid); sessions=len({x.get("session_id") for x in conversations if x.get("session_id")}); return {"business_id":bid,"business_name":businesses.data[0].get("business_name",""),"total_revenue":revenue,"closed_deals":deals,"conversion_rate":round(deals/sessions*100,2) if sessions else 0,"average_order_value":round(revenue/deals,2) if deals else 0,"total_conversations":sessions,"charts_data":[]}
