"""Dashboard APIs with explicit, normalized three-tier feature gates."""
...
def normalize_plan(value:Any)->str:
    value=str(value or "free").strip().lower(); return {"professional":"pro","pro_monthly":"pro","premium_monthly":"premium","paid":"pro"}.get(value,value if value in PLANS else "free")
def subscription_active(row:Dict[str,Any])->bool:
    plan=normalize_plan(row.get("plan_type")); raw=row.get("subscription_expires_at")
    if plan=="free" or not raw:return True
    try:
        exp=datetime.fromisoformat(str(raw).replace("Z","+00:00")); exp=exp if exp.tzinfo else exp.replace(tzinfo=timezone.utc); return exp>datetime.now(timezone.utc)
    except (TypeError,ValueError): return False
def get_user_plan(user_id:str)->Dict[str,Any]:
    """Load the whole profile row, avoiding failures when an older DB lacks optional columns."""
    try:
...
async def features(current_user:AuthUser=Depends(require_auth)):
    p=require_verified(current_user); plan=p["plan_type"]; return {"plan_type":plan,"subscription_active":p.get("subscription_active",True),**PLANS[plan],"upgrade_url":"/upgrade.html" if plan!="premium" else ""}
@router.get("/limits")
async def limits(current_user:AuthUser=Depends(require_auth)):
    p=require_verified(current_user); plan=p["plan_type"]; return {"plan_type":plan,"domain_limit":PLANS[plan]["domain_limit"],"api_key_limit":PLANS[plan]["api_key_limit"],"features":PLANS[plan]}
...
async def analytics(current_user:AuthUser=Depends(require_auth)):
    ...
    bid=businesses.data[0]["id"]; paid=supabase.table("orders").select("total").eq("business_id",bid).in_("status",["paid","shipped","delivered"]).execute().data or []; conv=supabase.table("conversations").select("session_id").eq("business_id",bid).execute().data or []; revenue=sum(float(x.get("total") or 0) for x in paid); deals=len(paid); sessions=len({x.get("session_id") for x in conv if x.get("session_id")}); return {"business_id":bid,"business_name":businesses.data[0]["business_name"],"total_revenue":round(revenue,2),"closed_deals":deals,"conversion_rate":round(deals/sessions*100,2) if sessions else 0,"average_order_value":round(revenue/deals,2) if deals else 0,"total_conversations":sessions,"charts_data":[]}
@router.get("/products")
async def products(current_user:AuthUser=Depends(require_auth)):
    ...
    rows=supabase.table("products").select("id,name,price,inventory_count,metadata").eq("business_id",businesses.data[0]["id"]).execute().data or []; return {"products":[{"id":x.get("id"),"name":x.get("name"),"price":x.get("price",0),"stock":x.get("inventory_count",-1),"category":(x.get("metadata") or {}).get("category","")} for x in rows],"total":len(rows)}
@router.get("/orders")
async def orders(current_user:AuthUser=Depends(require_auth)):
    ...
    return {"orders":rows if PLANS[p["plan_type"]]["orders"] else rows[:5],"total_count":len(rows) if PLANS[p["plan_type"]]["orders"] else len(rows[:5]),"locked":not PLANS[p["plan_type"]]["orders"],"lock_message":"ניהול הזמנות ומכירות זמין במסלולי PRO ו-PREMIUM.","upgrade_url":"/upgrade.html"}
