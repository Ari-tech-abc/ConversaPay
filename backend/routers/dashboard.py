...
def normalize_plan(value: Any) -> str:
    value = str(value or "free").strip().lower()
    aliases = {"pro_monthly": "pro", "professional": "pro", "premium_monthly": "premium", "paid": "pro"}
    return aliases.get(value, value if value in PLANS else "free")

def subscription_active(row: Dict[str, Any]) -> bool:
    plan = normalize_plan(row.get("plan_type"))
    if plan == "free": return True
    raw = row.get("subscription_expires_at")
    if not raw: return True
    try:
        expires = datetime.fromisoformat(str(raw).replace("Z", "+00:00"))
        if not expires.tzinfo: expires = expires.replace(tzinfo=timezone.utc)
...
def get_user_plan(user_id: str) -> Dict[str, Any]:
    ...
    row = result.data or {}
    requested = normalize_plan(row.get("plan_type"))
    active = subscription_active({**row, "plan_type": requested})
    plan = requested if active else "free"
    return {**row, "plan_type": plan, "subscription_active": active}
...
async def profile(current_user: AuthUser = Depends(require_auth)):
    ...
    return {"user_id": current_user.user_id, "email": current_user.email, "plan_type": p["plan_type"], "email_verified": p.get("email_verified", False), "subscription_expires_at": p.get("subscription_expires_at"), "subscription_active": p.get("subscription_active", True)}

@router.get("/features")
async def features(current_user: AuthUser = Depends(require_auth)):
    p = require_verified(current_user); plan = p["plan_type"]
    return {"plan_type": plan, "subscription_active": p.get("subscription_active", True), **PLANS[plan], "upgrade_url": "/upgrade.html" if plan != "premium" else ""}
...
async def orders(current_user: AuthUser = Depends(require_auth)):
    ...
    if not PLANS[p["plan_type"]]["orders"]: return {"orders": rows[:5], "total_count": len(rows[:5]), "locked": True, "lock_message": "ניהול הזמנות ומכירות זמין במסלולי PRO ו-PREMIUM.", "upgrade_url": "/upgrade.html"}
