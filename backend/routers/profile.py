...
async def change_password(payload: PasswordPayload, current_user: AuthUser = Depends(get_current_user)):
    if len(payload.new_password) < 8: raise HTTPException(400, "Password must be at least 8 characters")
    try:
        # Validate the current password through Supabase Auth before changing it.
...
async def revoke_other_sessions(current_user: AuthUser = Depends(get_current_user)):
    try:
        supabase.auth.admin.sign_out(current_user.user_id, "others")
    except Exception:
        pass
...
async def update_notifications(payload: PreferencesPayload, current_user: AuthUser = Depends(get_current_user)):
    row = _safe_update(current_user.user_id, {"notification_preferences": payload.model_dump()})
    return {"preferences": _profile_view(row).get("notification_preferences")}

@router.get("/billing")
async def billing_summary(current_user: AuthUser = Depends(get_current_user)):
    row = get_user_profile(current_user.user_id) or {}
    plan = row.get("plan_type") or "free"
    limits = {"free": 1000, "pro": 25000, "premium": 100000}
    used = 0
    try:
        result = supabase.table("usage_logs").select("id", count="exact").eq("user_id", current_user.user_id).execute()
        used = result.count or 0
    except Exception: pass
    return {"plan_type": plan, "subscription_status": row.get("subscription_status") or "active", "subscription_expires_at": row.get("subscription_expires_at"), "usage": {"used": used, "limit": limits.get(plan, 1000)}, "manage_url": "/upgrade", "invoices_url": "/upgrade#invoices"}

@router.get("/export")
async def export_account(current_user: AuthUser = Depends(get_current_user)):
    profile = get_user_profile(current_user.user_id) or {}
    return {"exported_at": _now(), "user": {"id": current_user.user_id, "email": current_user.email}, "profile": _profile_view(profile)}
