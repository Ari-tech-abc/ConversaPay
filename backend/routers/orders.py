...
from backend.middleware.auth import require_business_owner_for_business_id
...
def _catalog_order(request, business_uuid):
    ...
    for requested in request.items:
        key = requested.item_key.strip().upper(); product = products[key]
        ...
        price = float(product["price"]); subtotal += price * quantity
        items.append({"product_id": product["id"], "item_key": product["item_key"], "name": product["name"], "quantity": quantity, "price": price, "payment_link": product.get("payment_link")})
...
def _order_payload(request, business_uuid):
    ...
    return {"business_id": business_uuid, "customer_id": request.customer_id, "conversation_id": request.conversation_id, "order_number": _order_number(), "status": OrderStatus.PENDING.value, "payment_status": "pending", "subtotal": subtotal, "tax": 0, "total": subtotal, "currency": currency, "items": items, "customer_info": request.customer_info, "shipping_address": request.shipping_address, "notes": request.notes, "created_at": datetime.utcnow().isoformat()}
...
async def get_public_order(order_id: str):
    """Return only checkout-safe order fields for the public payment page."""
    ...
    return {"id": order.get("id"), "order_number": order.get("order_number"), "status": order.get("status"), "total": order.get("total"), "currency": order.get("currency"), "items": items, "payment_link": items[0].get("payment_link") if len(items) == 1 and isinstance(items[0], dict) else None, "created_at": order.get("created_at")}
...
async def get_order(order_id: str, current_user: AuthUser = Depends(require_auth)):
    ...
    if not owner.data: raise HTTPException(403, "Access denied")
