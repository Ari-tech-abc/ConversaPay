...
async def get_businesses(current_user: AuthUser = Depends(require_auth)):
    result = supabase.table("businesses").select("*").eq("owner_id", current_user.user_id).order("created_at", desc=True).execute(); return [BusinessResponse(**x) for x in (result.data or [])]
