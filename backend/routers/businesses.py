...
async def create_business(request: BusinessCreate, current_user: AuthUser = Depends(require_auth)):
    try:
        if supabase.table("businesses").select("id").eq("business_id", request.business_id).execute().data:
            raise HTTPException(400, "Business ID already exists")
        ...
        data = {"business_id": request.business_id, "business_name": request.business_name.strip(), "description": request.description, "owner_id": current_user.user_id, "subscription_tier": plan, "subscription_status": "active", "is_active": True}
        ...
        logger.error("Error creating business: %s", exc, exc_info=True)
        raise HTTPException(500, "Failed to create business")
...
async def update_business(business_id: str, request: BusinessUpdate, current_user: AuthUser = Depends(require_auth)):
    if not supabase.table("businesses").select("id").eq("id", business_id).eq("owner_id", current_user.user_id).execute().data:
        raise HTTPException(404, "Business not found")
...
async def get_business_stats(business_id: str, current_user: AuthUser = Depends(require_auth)):
    if not supabase.table("businesses").select("id").eq("id", business_id).eq("owner_id", current_user.user_id).execute().data:
        raise HTTPException(404, "Business not found")
    conv = supabase.table("conversations").select("id", count="exact").eq("business_id", business_id).execute()
    orders = supabase.table("orders").select("total,status").eq("business_id", business_id).execute()
    customers = supabase.table("customers").select("id", count="exact").eq("business_id", business_id).execute()
    rows = orders.data or []
    revenue = sum(float(x.get("total") or 0) for x in rows)
    total_conv = conv.count if conv.count is not None else len(conv.data or [])
    return {"business_id": business_id, "total_conversations": total_conv, "total_orders": len(rows), "total_revenue": round(revenue, 2), "total_customers": customers.count if customers.count is not None else len(customers.data or []), "conversion_rate": round(len(rows) / total_conv * 100, 2) if total_conv else 0}
