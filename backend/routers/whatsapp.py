...
async def verify_webhook(request: Request):
    ...
    if not result.data or not _premium(result.data):
        raise HTTPException(status_code=403, detail="Verification token not found")
...
async def handle_whatsapp_webhook(request: Request):
    try:
        ...
        if not phone_id or message.get("type") != "text":
            return {"status":"ok"}
        profile_result = supabase.table("profiles").select("*").eq("whatsapp_phone_number_id", phone_id).eq("plan_type", "premium").maybe_single().execute()
        ...
        if profile.get("whatsapp_access_token"):
            await send_whatsapp_message(customer_phone, answer.get("response", ""), profile["whatsapp_access_token"], phone_id)
        return {"status":"ok"}
    except Exception:
