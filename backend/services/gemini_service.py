...
class GeminiService:
    def __init__(self): self.client=genai.Client(api_key=settings.GEMINI_API_KEY); self.model_name="gemini-2.5-flash-lite"
    ...
    async def chat(self,business_id,session_id,message,business_data,products,customer_context=None,conversation_history=None):
        try:
            response=self.client.models.generate_content(model=self.model_name,contents=self._build_prompt(business_data.get('business_name','העסק'),business_data.get('description',''),products,conversation_history or [],message),config=types.GenerateContentConfig(max_output_tokens=256,thinking_config=types.ThinkingConfig(thinking_budget=0))); bot_text=response.text or "איך אוכל לעזור?"; action_data=None; intent="chat"
