...
def _is_email_verified(user) -> bool:
    """Trust Supabase Auth's verification state, not a profile-side flag."""
    ...
    if is_verified is None and isinstance(user, dict):
        is_verified = user.get("is_verified")
    return bool(confirmed_at or is_verified is True)
...
async def get_current_user(request: Request, credentials: HTTPAuthorizationCredentials = Depends(security)) -> AuthUser:
    try:
        ...
        user = response.user
        return AuthUser(user.id, user.email or "", _is_email_verified(user))
...
async def get_current_user_optional(request: Request) -> Optional[AuthUser]:
    ...
    if not auth.startswith("Bearer "):
        return None
    try:
        response = supabase_client.auth.get_user(auth.split(" ", 1)[1])
        if not response or not response.user:
            return None
        user = response.user
        return AuthUser(user.id, user.email or "", _is_email_verified(user))
...
class RoleChecker:
    ...
    def __call__(self, user: AuthUser = Depends(get_current_user)) -> AuthUser:
        ...
        try:
            ...
            if not profile.data or profile.data[0].get("role", "user") not in self.allowed_roles:
                raise HTTPException(403, "Insufficient permissions")
            return user
        except HTTPException:
            raise
        except Exception as exc:
            logger.error("Role check error: %s", exc, exc_info=True)
            raise HTTPException(500, "Failed to verify role")
...
def require_business_owner_for_business_id(business_id: str, current_user: AuthUser) -> str:
    ...
    if not result.data:
        raise HTTPException(403, "Unauthorized access to this business resources")
    return str(result.data[0]["id"])


require_admin = RoleChecker(["admin", "super_admin"])
require_business_owner = RoleChecker(["admin", "super_admin", "business_owner"])
