...
class SubscriptionStatus(str, Enum): ACTIVE="active"; CANCELED="canceled"; PAST_DUE="past_due"
class ChannelType(str, Enum): WEB="web"; WHATSAPP="whatsapp"; TELEGRAM="telegram"; API="api"
class ConversationStatus(str, Enum): ACTIVE="active"; CLOSED="closed"; ARCHIVED="archived"
class OrderStatus(str, Enum): PENDING="pending"; PROCESSING="processing"; PAID="paid"; SHIPPED="shipped"; DELIVERED="delivered"; CANCELED="canceled"; REFUNDED="refunded"
class PaymentStatus(str, Enum): PENDING="pending"; PROCESSING="processing"; SUCCEEDED="succeeded"; FAILED="failed"; CANCELED="canceled"; REFUNDED="refunded"
...
class BusinessBase(BaseModel):
    business_id: str = Field(..., min_length=3, max_length=50, pattern="^[a-z0-9_]+$"); business_name: str = Field(..., min_length=1, max_length=255); description: Optional[str] = None
class BusinessCreate(BusinessBase): pass
class BusinessUpdate(BaseModel): business_name: Optional[str]=Field(None,min_length=1,max_length=255); description: Optional[str]=None; settings: Optional[Dict[str,Any]]=None; is_active: Optional[bool]=None
...
class CustomerResponse(CustomerBase): model_config=ConfigDict(from_attributes=True); id:str; business_id:str; total_purchases:Decimal; purchase_count:int; last_purchase_at:Optional[datetime]=None; created_at:datetime; updated_at:datetime
...
class ConversationResponse(ConversationBase): model_config=ConfigDict(from_attributes=True); id:str; business_id:str; customer_id:Optional[str]=None; session_id:str; started_at:datetime; ended_at:Optional[datetime]=None; created_at:datetime; updated_at:datetime
...
class MessageResponse(MessageBase): model_config=ConfigDict(from_attributes=True); id:str; conversation_id:str; created_at:datetime
class OrderItem(BaseModel): product_id:Optional[str]=None; item_key:str; name:str; quantity:int=Field(...,ge=1); price:Decimal=Field(...,ge=0)
class OrderBase(BaseModel): status:OrderStatus=OrderStatus.PENDING; subtotal:Decimal=Field(...,ge=0); tax:Decimal=Field(default=Decimal("0.00"),ge=0); total:Decimal=Field(...,ge=0); currency:str=Field(default="ILS",min_length=3,max_length=3); items:List[OrderItem]; customer_info:Dict[str,Any]={}; shipping_address:Optional[Dict[str,Any]]=None; notes:Optional[str]=None
...
class PaymentResponse(PaymentBase): model_config=ConfigDict(from_attributes=True); id:str; business_id:str; order_id:Optional[str]=None; paid_at:Optional[datetime]=None; created_at:datetime; updated_at:datetime
...
class ProfileResponse(ProfileBase): model_config=ConfigDict(from_attributes=True); id:str; user_id:str; plan_type:str="free"; subscription_expires_at:Optional[datetime]=None; whatsapp_phone_number_id:Optional[str]=None; whatsapp_access_token:Optional[str]=None; whatsapp_verify_token:Optional[str]=None; created_at:datetime; updated_at:datetime
...
class AnalyticsOverviewResponse(BaseModel): total_revenue:Decimal=Field(default=Decimal("0.00")); closed_deals_count:int=Field(default=0,ge=0); average_order_value:Decimal=Field(default=Decimal("0.00"),ge=0); total_conversations:int=Field(default=0,ge=0)
