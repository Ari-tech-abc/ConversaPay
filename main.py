...
class DualCORSMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self,request:Request,call_next):
        origin=request.headers.get("origin");is_public=request.url.path.startswith(self.PUBLIC_PREFIXES);is_allowed_origin=bool(origin and origin in settings.cors_origins_list)
        if request.method=="OPTIONS":
            if not is_public and not is_allowed_origin:return Response(status_code=403)
            response=Response(status_code=204)
        else:response=await call_next(request)
        if is_public:response.headers["Access-Control-Allow-Origin"]="*"
        elif is_allowed_origin:response.headers.update({"Access-Control-Allow-Origin":origin,"Access-Control-Allow-Credentials":"true"})
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self,request:Request,call_next):
        response=await call_next(request);response.headers.setdefault("Content-Security-Policy",self.CSP);response.headers.setdefault("Cross-Origin-Opener-Policy","same-origin");response.headers.setdefault("Permissions-Policy","camera=(), microphone=(), geolocation=(), payment=()");response.headers.setdefault("Referrer-Policy","strict-origin-when-cross-origin");response.headers.setdefault("X-Content-Type-Options","nosniff");response.headers.setdefault("X-Frame-Options","SAMEORIGIN")
        content_type=response.headers.get("content-type","").lower();path=request.url.path
        if path.startswith(settings.API_PREFIX) or "text/html" in content_type:response.headers.setdefault("Cache-Control","no-store")
        elif path.startswith(("/frontend/","/images/","/static/")):response.headers.setdefault("Cache-Control","public, max-age=86400, stale-while-revalidate=604800")
        if settings.is_production:response.headers.setdefault("Strict-Transport-Security","max-age=31536000; includeSubDomains")
        return response

app.add_middleware(CorrelationIdMiddleware);app.add_middleware(DualCORSMiddleware);app.add_middleware(SecurityHeadersMiddleware)

@app.get("/health",include_in_schema=False)
async def health():return {"status":"healthy","version":"2.3.0","environment":settings.ENVIRONMENT}
@app.get("/ready",include_in_schema=False)
async def readiness():return {"status":"ready","environment":settings.ENVIRONMENT}
...
ROOT_DIR=Path(__file__).resolve().parent;STATIC_DIR=ROOT_DIR/"backend"/"static";FRONTEND_DIR=ROOT_DIR/"frontend";HTML_DIR=FRONTEND_DIR/"html";IMAGES_DIR=FRONTEND_DIR/"images";SITE_BUILDER_DIR=ROOT_DIR/"conversapay-site-builder"/"frontend";ADMIN_SECRET=settings.ADMIN_SECRET_PATH.strip();BRAND_LOGO_SRC="/frontend/images/conversapay_logo_whitebg.png";FAVICON_SRC="/frontend/images/favicon-32x32.png";DESIGN_SYSTEM_LINK='<link rel="stylesheet" href="/frontend/html/design-system.css">';LANDING_FIXES_LINK='<link rel="stylesheet" href="/frontend/html/landing-fixes.css">';BRANDING_STYLE='<style id="conversapay-branding">.cp-brand,.brand,.legal-header>a{display:inline-flex;align-items:center;min-height:38px}.cp-brand-logo,.brand-logo,.legal-header .cp-brand-logo{display:block;width:auto;height:38px;max-width:min(220px,55vw);object-fit:contain}@media(max-width:760px){.cp-brand-logo,.brand-logo,.legal-header .cp-brand-logo{height:32px;max-width:180px}}</style>'

def html_path(name:str)->Path:
    path=(HTML_DIR/name).resolve()
    if path.parent!=HTML_DIR.resolve():raise ValueError("Invalid HTML path")
    return path

def brand_markup(page:str)->str:
    def replace_cp_brand(match:re.Match)->str:
        tag=match.group(0);href_match=re.search(r'href=["\']([^"\']+)',tag,re.IGNORECASE);href=href_match.group(1) if href_match else "/"
    ...
    if DESIGN_SYSTEM_LINK not in page:page=page.replace("</head>",f"{DESIGN_SYSTEM_LINK}</head>",1)
    if 'class="home-page"' in page and LANDING_FIXES_LINK not in page:page=page.replace("</head>",f"{LANDING_FIXES_LINK}</head>",1)
    if 'id="conversapay-branding"' not in page:page=page.replace("</head>",f"{BRANDING_STYLE}</head>",1)
    return page

def branded_file(path:Path,status_code:int=200)->HTMLResponse:return HTMLResponse(brand_markup(path.read_text(encoding="utf-8")),status_code=status_code)
...
async def serve_admin_dashboard(secret_path:str):
    if not ADMIN_SECRET or not secrets_match(secret_path,ADMIN_SECRET):raise HTTPException(status_code=404,detail="Not found")
    page=html_path("admin-dashboard.html").read_text(encoding="utf-8").replace('href="/admin-change-password.html"','href="change-password"');return HTMLResponse(brand_markup(page))
...
app.mount("/static",StaticFiles(directory=STATIC_DIR),name="static");app.mount("/images",StaticFiles(directory=IMAGES_DIR),name="images");app.mount("/frontend",StaticFiles(directory=FRONTEND_DIR),name="frontend")
@app.get("/conversapay-ui.css",include_in_schema=False)
async def conversapay_ui_stylesheet():return FileResponse(html_path("conversapay-ui.css"),media_type="text/css")
@app.get("/")
async def root():return branded_file(html_path("home.html"))
@app.get("/dashboard")
@app.get("/dashboard.html")
async def dashboard_page():
    page=html_path("dashboard.html").read_text(encoding="utf-8");wordpress_link='<a class="nav-pill" href="/wordpress" aria-label="פתיחת מסך תפעול WordPress">WordPress</a>'
    if wordpress_link not in page and "</nav>" in page:page=page.replace("</nav>",f"{wordpress_link}</nav>",1)
    additions=(DESIGN_SYSTEM_LINK,'<link rel="stylesheet" href="/frontend/html/saas-overrides.css">','<script src="/frontend/js/business-onboarding.js" defer></script>','<script src="/frontend/js/premium-builder-link.js" defer></script>')
    for addition in additions:
        if addition not in page:page=page.replace("</head>",f"{addition}</head>",1)
    return HTMLResponse(brand_markup(page))
...
def make_page_handler(filename:str):
    async def page_handler():return branded_file(html_path(filename))
    return page_handler
for route,filename in PAGE_ROUTES.items():app.add_api_route(route,make_page_handler(filename),methods=["GET"],include_in_schema=False)
@app.get("/404")
@app.get("/404.html")
async def not_found_page():return branded_file(html_path("404.html"),status_code=404)
if ADMIN_SECRET:
    @app.get("/admin")
    @app.get("/admin.html")
    @app.get("/admin/login")
    ...
    async def admin_hidden():raise HTTPException(status_code=404,detail="Not found")
    @app.get("/admin/{path:path}")
    async def admin_catchall_hidden(path:str):raise HTTPException(status_code=404,detail="Not found")
else:
    @app.get("/admin")
    @app.get("/admin.html")
    async def admin_page():return branded_file(html_path("admin-dashboard.html"))
    @app.get("/admin/login")
    @app.get("/admin-login.html")
    async def admin_login_page():return branded_file(html_path("admin-login.html"))
    @app.get("/admin/change-password")
    @app.get("/admin-change-password.html")
    async def admin_change_password_page():return branded_file(html_path("admin-change-password.html"))
@app.get("/site-builder")
@app.get("/site-builder.html")
async def site_builder_page():return branded_file(SITE_BUILDER_DIR/"index.html")
@app.get("/robots.txt",include_in_schema=False)
async def robots():return FileResponse(STATIC_DIR/"robots.txt",media_type="text/plain")
@app.get("/sitemap.xml",include_in_schema=False)
async def sitemap():return FileResponse(STATIC_DIR/"sitemap.xml",media_type="application/xml")
@app.exception_handler(404)
async def not_found(request:Request,exc:HTTPException):
    if "text/html" in request.headers.get("accept",""):return branded_file(html_path("404.html"),status_code=404)
...
async def internal_error(_:Request,exc:Exception):
    logger.exception("Unhandled server error",exc_info=exc);return JSONResponse(status_code=500,content={"error":"Internal server error","detail":"An unexpected error occurred"})
