...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    CSP="; ".join(("default-src 'self'","base-uri 'self'","object-src 'none'","frame-ancestors 'self'","form-action 'self' https://accounts.google.com","img-src 'self' data: https:","font-src 'self' data: https://fonts.gstatic.com","style-src 'self' 'unsafe-inline' https://fonts.googleapis.com","script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://accounts.google.com","connect-src 'self' https://*.supabase.co wss://*.supabase.co https://accounts.google.com https://*.googleapis.com","upgrade-insecure-requests"))
    async def dispatch(self,request:Request,call_next):
        response=await call_next(request)
        is_auth_page = request.url.path in ("/login", "/register", "/auth/callback")
        coop_value = "same-origin-allow-popups" if is_auth_page else "same-origin"
        for key,value in (("Content-Security-Policy",self.CSP),("Cross-Origin-Opener-Policy",coop_value),("Permissions-Policy","camera=(), microphone=(), geolocation=(), payment=()"),("Referrer-Policy","strict-origin-when-cross-origin"),("X-Content-Type-Options","nosniff"),("X-Frame-Options","SAMEORIGIN")):response.headers.setdefault(key,value)
        if settings.is_production:response.headers.setdefault("Strict-Transport-Security","max-age=31536000; includeSubDomains")
        return response
class PublicEndpointSafetyNetMiddleware(BaseHTTPMiddleware):
    """Defense-in-depth only. Per-endpoint rate limiting (chat.py, widget.py)
    is the real control and is scoped per-business; this is a coarse,
    generous, IP-only backstop so that a *future* public route added under
    PUBLIC_PREFIXES without its own check_rate_limit() call doesn't end up
    completely unbounded. Do not rely on this alone for chat/widget."""
    PUBLIC_PREFIXES=DualCORSMiddleware.PUBLIC_PREFIXES
    _limiter=RateLimiter(requests_per_minute=120,name="public_safety_net")
    async def dispatch(self,request:Request,call_next):
        if request.url.path.startswith(self.PUBLIC_PREFIXES) and not self._limiter.is_allowed(get_client_ip(request)):
            return JSONResponse(status_code=429,content={"error":"Rate limit exceeded","message":"Too many requests. Please try again later."})
        return await call_next(request)
app.add_middleware(CorrelationIdMiddleware); app.add_middleware(AuthRateLimitMiddleware); app.add_middleware(PublicEndpointSafetyNetMiddleware); app.add_middleware(DualCORSMiddleware); app.add_middleware(SecurityHeadersMiddleware)
