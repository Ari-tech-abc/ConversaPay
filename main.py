...
class DualCORSMiddleware(BaseHTTPMiddleware):
    """Apply explicit CORS rules without reflecting untrusted origins."""

    PUBLIC_PREFIXES = (
        f"{settings.API_PREFIX}/chat",
        f"{settings.API_PREFIX}/widget",
        f"{settings.API_PREFIX}/webhooks/",
        f"{settings.API_PREFIX}/orders/",
        f"{settings.API_PREFIX}/site-builder/",
    )
    ALLOWED_METHODS = "GET,POST,PUT,PATCH,DELETE,OPTIONS"
    ALLOWED_HEADERS = (
        "Content-Type,Authorization,X-Builder-Token,X-Widget-Key,"
        "X-Webhook-Signature,X-Correlation-ID"
    )
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    """Set browser security policy and conservative caching by response type."""
...
for router, route_prefix, tag in (
    (onboarding.router, prefix, "authentication-onboarding"),
    (auth.router, f"{prefix}/auth", "authentication"),
    (profile.router, f"{prefix}/profile", "profile"),
    (subscription.router, prefix, "subscription"),
    (businesses.router, prefix, "businesses"),
    (products.router, prefix, "products"),
    (chat.router, prefix, "chat"),
    (orders.router, prefix, "orders"),
    (payments.router, prefix, "payments"),
    (logs.router, prefix, "logs"),
    (webhooks.router, prefix, "webhooks"),
    (analytics.router, prefix, "analytics"),
    (widget.router, prefix, "widget"),
    (dashboard.router, prefix, "dashboard"),
    (admin.router, prefix, "admin"),
    (admin_password_router, prefix, "admin-security"),
    (api_keys.router, prefix, "api-keys"),
    (site_builder.router, prefix, "site-builder"),
    (stripe_webhook_router, prefix, "stripe-webhook"),
    (whatsapp_webhook_router, prefix, "whatsapp-webhook"),
):
...
.cp-brand-logo,.brand-logo,.legal-header .cp-brand-logo{display:block;width:auto;height:38px;max-width:min(220px,55vw);object-fit:contain}
...
async def serve_admin_dashboard(secret_path: str):
    ...
    page = html_path("admin-dashboard.html").read_text(encoding="utf-8")
