...
auth_get_user_profile = auth.get_user_profile
auth_update_profile_row = auth.update_profile_row
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        response.headers.setdefault("Content-Security-Policy", self.CSP); response.headers.setdefault("Cross-Origin-Opener-Policy", "same-origin"); response.headers.setdefault("Permissions-Policy", "camera=(), microphone=(), geolocation=(), payment=()"); response.headers.setdefault("Referrer-Policy", "strict-origin-when-cross-origin"); response.headers.setdefault("X-Content-Type-Options", "nosniff"); response.headers.setdefault("X-Frame-Options", "SAMEORIGIN")
...
async def readiness():
    ...
    if settings.REDIS_URL:
        ...
        except Exception as exc: logger.error("Readiness Redis check failed: %s",exc); checks["redis"]="failed"; failures["redis"]=str(exc)
...
for selected_router,route_prefix,tag in ((onboarding.router,prefix,"authentication-onboarding"),(safe_auth_router,prefix,"authentication"),(auth.router,f"{prefix}/auth","authentication"),(profile.router,f"{prefix}/profile","profile"),(subscription.router,prefix,"subscription"),(businesses.router,prefix,"businesses"),(products.router,prefix,"products"),(chat.router,prefix,"chat"),(orders.router,prefix,"orders"),(payments.router,prefix,"payments"),(logs.router,prefix,"logs"),(webhooks.router,prefix,"webhooks"),(analytics.router,prefix,"analytics"),(widget.router,prefix,"widget"),(dashboard.router,prefix,"dashboard"),(admin.router,prefix,"admin"),(admin_password_router,prefix,"admin-security"),(api_keys.router,prefix,"api-keys"),(site_builder.router,prefix,"site-builder"),(stripe_webhook_router,prefix,"stripe-webhook"),(whatsapp_webhook_router,prefix,"whatsapp-webhook")): app.include_router(selected_router,prefix=route_prefix,tags=[tag])
