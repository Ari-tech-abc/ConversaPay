...
def build_prompt(r: GenerateRequest) -> str:
    return f"Return only valid JSON for a landing page. Language: same as prompt. Business: {r.business_name}. Industry: {r.industry}. Vibe: {r.vibe}. Description: {r.prompt}. Sections: {', '.join(r.sections)}. Include hero, features, products, testimonials, theme and seo."

def render(data: Dict[str, Any]) -> str:
    hero = data.get("hero") or {}; theme = data.get("theme") or {}
    primary = esc(theme.get("primaryColor", "#635bff")); bg = esc(theme.get("backgroundColor", "#f7f7f2")); text = esc(theme.get("textColor", "#1d2433")); muted = esc(theme.get("secondaryColor", "#64748b"))
...
async def require_builder_token(token: Optional[str]) -> None:
    ...
    try:
        ...
        if response.status_code != 200:
            raise HTTPException(response.status_code, "Invalid, expired, or already used builder token")
    except HTTPException:
        raise
    except Exception as exc:
        logger.error("Builder token validation failed: %s", exc)
        raise HTTPException(503, "Builder authorization service unavailable")
